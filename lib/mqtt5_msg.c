...
static mqtt5_user_property_handle_t mqtt5_msg_get_user_property(uint8_t *buffer, size_t buffer_length)
{
    ...
    while (property_offset < buffer_length) {
        ...
        default:
            ESP_LOGW(TAG, "Unknown property id 0x%02x", property_id);
            goto err;
        }
    }
                                         ...
                                         mqtt5_user_property_handle_t *user_property)
{
    ...
    *property_len = get_variable_len(buffer, offset, buffer_length, &len_bytes);
    ...
    while (property_offset < *property_len) {
        ...
        default:
            ESP_LOGW(TAG, "Unknown publish property id 0x%02x", property_id);
            return NULL;
        }
    }
    ...
    *reason_code = buffer[offset ++]; //reason code
    ...
    while (property_offset < property_len) {
        ...
        case MQTT5_PROPERTY_RECEIVE_MAXIMUM:
            MQTT5_CONVERT_ONE_BYTE_TO_TWO(resp_property->receive_maximum, property[property_offset ++],
                                          property[property_offset ++])
        ...
        case MQTT5_PROPERTY_MAXIMUM_PACKET_SIZE:
            MQTT5_CONVERT_ONE_BYTE_TO_FOUR(resp_property->maximum_packet_size, property[property_offset ++],
                                           property[property_offset ++], property[property_offset ++], property[property_offset ++])
        ...
        case MQTT5_PROPERTY_TOPIC_ALIAS_MAXIMIM:
            MQTT5_CONVERT_ONE_BYTE_TO_TWO(resp_property->topic_alias_maximum, property[property_offset ++],
                                          property[property_offset ++])
        ...
        default:
            ESP_LOGW(TAG, "Unknown connack property id 0x%02x", property_id);
            return ESP_FAIL;
        }
    }
                                    ...
                                    uint16_t *message_id, const esp_mqtt5_subscribe_property_config_t *property)
{
    ...
    for (int topic_number = 0; topic_number < size; ++topic_number) {
        ...
        if (property && property->is_share_subscribe) {
            uint16_t shared_topic_size = strlen(topic_list[topic_number].filter) + strlen(MQTT5_SHARED_SUB) + strlen(
                                             property->share_name);
